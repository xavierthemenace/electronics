...
const mA = (a) => `${(a * 1000).toFixed(1)} mA`;
const V = (v) => `${v.toFixed(2)} V`;

/**
 * Run ERC against a project + DC solve result.
 * @param {import("./model.js").CircuitProject} project
 * @param {ReturnType<typeof import("./solver.js").solveDC>} solve
 * @returns {Violation[]}
 */
export function runERC(project, solve) {
  /** @type {Violation[]} */
  const v = [];
  ...
  if (!solve || !solve.ok || !hasGround) return v;

  // --- Per-component checks --------------------------------------------------------
  const nodeV = solve.nodeVoltages; // 0-indexed for node 1..

  // Build helpers.
  const findDevice = (cid) => solve.devBranches.find((d) => d.compId === cid);
  ...
  for (const c of project.components) {
    ...
    if (c.type === "led") {
      ...
      const i = d?.current ?? 0;
      const max = params.maxForwardCurrent ?? 0.02;
      if (Math.abs(i) > max) {
        v.push({
          severity: "error",
    ...
    if (c.type === "resistor") {
      const d = findDevice(c.id);
      const p = d?.power ?? 0;
      const rating = params.powerRating ?? 0.25;
      if (p > rating * 1.05) {
        v.push({
          severity: "warning",
          code: "R_POWER",
