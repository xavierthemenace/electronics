...
function report(label, prj) {
  ...
  for (const n of nl.nets) {
    console.log(`  ${n.name.padEnd(6)} (node ${n.node})${n.isGround ? "  ⏚ GND" : ""}  ` +
      n.members.map((m) => `${m.compId}.${m.pinId}`).join(", "));
  }
  ...
  if (prj.components.some((x) => x.id === "r1")) {
    const r = prj.components.find((x) => x.id === "r1");
    const led = prj.components.find((x) => x.id === "l1");
    const rNl = nl.nets.find((n) => n.members.some((m) => m.compId === "r1" && m.pinId === "1"));
    const rCr = nl.nets.find((n) => n.members.some((m) => m.compId === "r1" && m.pinId === "2"));
    ...
    const lNl = nl.nets.find((n) => n.members.some((m) => m.compId === "l1" && m.pinId === "a"));
    const lCr = nl.nets.find((n) => n.members.some((m) => m.compId === "l1" && m.pinId === "k"));
