...
import { connectivityGroups } from '../core/breadboard.js';
...
export function BreadboardPanel() {
  ...
  const topology = useBreadboardStore(s => s.topology);
  const jumper = useBreadboardStore(s => s.jumper);
  const place = useBreadboardStore(s => s.place);
  const reset = useBreadboardStore(s => s.reset);
  ...
  const groups = useMemo(() => connectivityGroups(topology), [topology]);
  const placementsByHole = useMemo(() => new Map(topology.placements.map(p => [p.holeId, `${p.componentId}:${p.pinId}`])), [topology.placements]);
  const componentList = [...components.values()];

  const pinsForType = (type: string): string[] => ({
    resistor: ['1', '2'], led: ['a', 'k'], capacitor: ['1', '2'], diode: ['a', 'k'],
    'dc-source': ['plus', 'minus'], ground: ['gnd'],
    'arduino-uno': ['d2', 'd3', 'd4', 'd5', 'd6', 'd7', 'd8', 'd9', 'd10', 'd11', 'd12', 'd13', 'a0', 'a1', 'a2', 'a3', 'a4', 'a5', '5v', 'gnd'],
  }[type] ?? []);
  ...
  const clickHole = (holeId: string) => {
    if (selectedComponent && selectedPin) {
      try { place(selectedComponent, selectedPin, holeId); setStatus(`Placed ${selectedComponent}:${selectedPin} at ${holeId}.`); }
      catch (err) { setStatus(String(err)); }
      return;
    }
    if (!firstHole) { setFirstHole(holeId); setStatus(`Selected ${holeId}. Click another hole to jumper it.`); return; }
    if (firstHole === holeId) return;
    try { jumper(firstHole, holeId); setStatus(`Jumper connected ${firstHole} → ${holeId}.`); }
    catch (err) { setStatus(String(err)); }
    setFirstHole(null);
  };

  const renderBank = (side: 'top' | 'bottom', bank: 'left' | 'right') => (
    <div style={{ display: 'grid', gridTemplateRows: `repeat(${topology.rows}, 18px)`, gap: 2 }}>
      {Array.from({ length: topology.rows }).map((_, row) => (
        <div key={`${side}-${bank}-${row}`} style={{ display: 'grid', gridTemplateColumns: `repeat(${topology.columns}, 18px)`, gap: 2 }}>
          {topology.holes.filter(h => h.side === side && h.bank === bank && h.row === row).map(h => {
  ...
  return (
    <div style={panel}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <div><strong>Breadboard Lab</strong><div style={{ color: '#8b949e', marginTop: 3 }}>30-row solderless topology • physical five-hole strips • project-persistent</div></div>
        <button onClick={() => { reset(); setFirstHole(null); setStatus('Board reset.'); }} style={{ border: '1px solid #30363d', background: '#21262d', color: '#e6edf3', padding: '6px 9px', borderRadius: 5 }}>Reset board</button>
      </div>
      <div style={card}>
      ...
      <div style={{ ...card, overflowX: 'auto' }}>
        <div style={{ minWidth: 520, display: 'grid', gridTemplateColumns: 'auto 1fr 1fr', gap: 14 }}>
          <div style={{ color: '#6e7681', fontFamily: 'var(--font-family-mono)' }}>{Array.from({ length: topology.rows }).map((_, i) => <div key={i} style={{ height: 20 }}>{String(i + 1).padStart(2, ' ')}</div>)}</div>
          <div><div style={{ textAlign: 'center', color: '#8b949e', marginBottom: 6 }}>BANK A</div>{renderBank('top', 'left')}<div style={{ height: 20 }} />{renderBank('bottom', 'left')}</div>
          <div><div style={{ textAlign: 'center', color: '#8b949e', marginBottom: 6 }}>BANK B</div>{renderBank('top', 'right')}<div style={{ height: 20 }} />{renderBank('bottom', 'right')}</div>
        </div>
      </div>
      <div style={card}>
        <strong>Connectivity</strong>
        <div style={{ color: '#8b949e', marginTop: 6 }}>Physical strips are electrically connected before jumpers are applied. Green holes contain component-pin placements.</div>
