...
const CATEGORIES = [
  ...
  { id: 'memory', name: 'Memory', icon: '▤' }, { id: 'digital', name: 'Digital', icon: '🔢' },
...
function CategorySection({ category, components, onDragStart }: { category: typeof CATEGORIES[0]; components: ReturnType<typeof listComponents>; onDragStart: (e: React.DragEvent, compType: string) => void; }) {
  ...
  return <div style={styles.categorySection}>
...
function ComponentItem({ comp, onDragStart }: { comp: ReturnType<typeof listComponents>[0]; onDragStart: (e: React.DragEvent, compType: string) => void; }) {
  ...
  return <div draggable onDragStart={e => onDragStart(e, comp.type)} style={{ ...styles.componentItem, opacity: modelled ? 1 : .5, cursor: modelled ? 'grab' : 'not-allowed' }} title={modelled ? comp.docs.description : `${comp.name} (planned)`}>
...
function getComponentIcon(type: string): string {
  const icons: Record<string, string> = {
    ground: '⏚', 'dc-source': '🔋', resistor: '▱', led: '💡', diode: '▶', capacitor: '||', inductor: '⌐', potentiometer: '⟲', zener: '⊥', 'bjt-npn': '◢', 'mosfet-n': 'Ⓜ', and: '&', not: '¬', 'arduino-uno': '🤖', 'analog-sensor': '◉', pushbutton: '●', 'dc-motor': '⚙', servo: '↻', 'lcd-1602': '▣', 'i2c-temp': '🌡', 'i2c-eeprom': '▤', 'spi-dac': '◈', 'spi-flash': '▥',
  };
  return icons[type] || '□';
}
...
export function ComponentPalette() {
  ...
  const handleDragStart = (e: React.DragEvent, compType: string) => {
    e.dataTransfer.setData('application/x-component-type', compType); e.dataTransfer.effectAllowed = 'copy';
    try { e.dataTransfer.setDragImage(e.currentTarget as HTMLElement, 12, 12); } catch { /* browser fallback */ }
  };
  return <div style={styles.palette}>
