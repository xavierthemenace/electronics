...
function ComponentItem({ comp, onDragStart }: {
  comp: ReturnType<typeof listComponents>[0];
  onDragStart: (e: React.DragEvent, compType: string) => void;
}) {
  ...
  return (
    <div
      ...
      style={{
        ...
        opacity: modelled ? 1 : 0.5,
        cursor: modelled ? 'grab' : 'not-allowed',
      }}
      title={modelled ? comp.docs.description : `${comp.name} (not yet simulated — can be placed but not analyzed)`}
    >
      <span style={styles.componentIcon}>{getComponentIcon(comp.type)}</span>
      <span style={styles.componentName}>{comp.name}</span>
      {!modelled && <span style={styles.plannedBadge}>Planned</span>}
    </div>
  );
}
...
export function ComponentPalette() {
  ...
  const handleDragStart = (e: React.DragEvent, compType: string) => {
    ...
    // Use a custom drag image so the palette item isn't a weird text rect
    try {
      e.dataTransfer.setDragImage(e.currentTarget as HTMLElement, 12, 12);
    } catch {
      // some browsers throw if already in a drag; ignore
    }
  };
  ...
  return (
    <div style={styles.palette}>
      <div style={styles.paletteHeader}>
        <h2 style={styles.paletteTitle}>Components</h2>
        <div style={styles.paletteHint}>Drag to canvas</div>
      </div>
